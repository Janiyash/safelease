...
export const getProperties = async (req: AuthRequest, res: Response, next: NextFunction): Promise<void> => {
  try {
    const { status, city, minRent, maxRent, page = 1, limit = 12 } = req.query;
    const query: Record<string, unknown> = {};

    if (req.user!.role === 'owner') query.owner = req.user!.userId;
    else if (req.user!.role === 'tenant') query.isApproved = true;
    // admin sees all
    ...
    const [properties, total] = await Promise.all([
      Property.find(query).populate('owner', 'name email').populate('tenant', 'name email').sort('-createdAt').skip(skip).limit(Number(limit)),
...
export const getPropertyById = async (req: AuthRequest, res: Response, next: NextFunction): Promise<void> => {
  try {
    ...
    if (req.user!.role === 'owner' && property.owner._id.toString() !== req.user!.userId) {
